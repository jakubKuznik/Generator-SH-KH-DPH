...
use crate::{
    date_func::{first_month_day, last_month_day},
    Config,
};
use std::fs::File;
use std::io::Read;
...
pub fn generate_dph(config: &Config) {
    ...
    let round_prijata = (config.prijata_zdanitelna_plneni_czk
        + config.prijata_zdanitelna_plneni_nad_limit_czk)
        .ceil() as i64;
    let round_dph_prijata = (round_prijata as f64 * 0.21).round() as i64;
    ...
    if let Some(dphdp3) = root.get_mut_child("DPHDP3") {
        ...
        if let Some(veta1) = dphdp3.get_mut_child("Veta1") {
            veta1.attributes.insert(
                "dan_psl23_e".to_string(),
                config.dph_prijeti_sluzeb_v_jinem_state_czk.to_string(),
            );
            veta1.attributes.insert(
                "p_sl23_e".to_string(),
                config.prijeti_sluzeb_v_jinem_state_czk.to_string(),
            );
        }

        // <Veta2> – hodnota plnění CZK
        if let Some(veta2) = dphdp3.get_mut_child("Veta2") {
            veta2.attributes.insert(
                "pln_sluzby".to_string(),
                config.hodnota_plneni_czk.to_string(),
            );
        }

        // <Veta4> – přijetí služeb v jiném státě
        if let Some(veta4) = dphdp3.get_mut_child("Veta4") {
            veta4.attributes.insert(
                "nar_zdp23".to_string(),
                config.prijeti_sluzeb_v_jinem_state_czk.to_string(),
            );
            veta4.attributes.insert(
            ...
            veta4
                .attributes
                .insert("odp_sum_nar".to_string(), dph_celkem.to_string());

            veta4
                .attributes
                .insert("odp_tuz23_nar".to_string(), round_dph_prijata.to_string());
            veta4
                .attributes
                .insert("pln23".to_string(), round_prijata.to_string());
        }

        // <Veta6> – celkový součet
        if let Some(veta6) = dphdp3.get_mut_child("Veta6") {
            veta6.attributes.insert(
                "dan_zocelk".to_string(),
                config.dph_prijeti_sluzeb_v_jinem_state_czk.to_string(),
            );
            veta6
                .attributes
                .insert("dano_no".to_string(), round_dph_prijata.to_string());
            veta6
                .attributes
                .insert("odp_zocelk".to_string(), dph_celkem.to_string());
        }
    }
